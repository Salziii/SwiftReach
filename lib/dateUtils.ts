...
export function isAvailableDay(day: Date): boolean {
  ...
  o.setSeconds(0);
  o.setMilliseconds(0);
  o.setDate(o.getDate() + NoMeetingsInAdvance);

  if (day < o) return false;
  if (day.getMonth() > new Date().getMonth() + monthsInAdvance) return false;
  return true;
...
export function generateTimespans(
  start: Date,
  end: Date,
  duration: number,
  ...
  const spans = [];
  for (
    let span = start.getTime();
    span <= end.getTime() - (duration + pauseDuration);
    span += duration + pauseDuration
  ) {
    if (!isInPast(new Date(span)))
      spans.push({ start: new Date(span), end: new Date(span + duration) });
  }
  return spans;
}
...
export function overlaps(timespan: any, appointments: any[]) {
  for (
    ...
    if (
      (appointment.start >= timespan.start && appointment.start < timespan.end) ||
      (appointment.end > timespan.start && appointment.end <= timespan.end) ||
      (appointment.start < timespan.start && appointment.end > timespan.end)
    )
      return true;
  }
  return false;
