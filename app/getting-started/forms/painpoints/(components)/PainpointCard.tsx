...
const PainpointCard = ({ painpoint, selected, handleClick, index }: { painpoint: any, selected: boolean, handleClick:any, index: number }) => {
 ...
 return (
  <div key={index} onClick={()=>handleClick(index)} className={`transition transform hover:-translate-y-1 h-32 py-3 px-4 flex justify-start bg-transparent border-2 ${selected ? "border-primary-foreground scale-[1.025]" : ""} hover:opacity-80 shadow-lg hover:shadow-sm rounded-lg cursor-pointer`}>
